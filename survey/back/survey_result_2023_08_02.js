...
async function surveyCart(type) {
  ...
  let _survey_sampoo_product_no = survey_shampoo_no;
  let _survey_treatment_product_no = survey_treatment_no;


  if (survey_discount_type && survey_discount_type == "employees") {
    let employees_set_product = survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_set_product");
    if (employees_set_product) {
      _survey_set_product_no = employees_set_product.product_no;
    }
    let employees_sampoo_product = survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_sampoo");
    if (employees_sampoo_product) {
      _survey_sampoo_product_no = employees_sampoo_product.product_no;
    }
    let employees_treatment_product = survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_treatment");
    if (employees_treatment_product) {
      _survey_treatment_product_no = employees_treatment_product.product_no;
    }
  }
  ...
  for (const product of product_list) {
    ...
    if (product_no != _survey_set_product_no) {
      ...
      let shampoo_set_param = {
        product_no: _survey_sampoo_product_no,
...
async function surveySetHairCareCart(
  ...
  let _survey_sampoo = survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "sampoo");
  let _survey_treatment = survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "treatment");

  if (survey_discount_type && survey_discount_type == "employees") {
    _survey_sampoo = survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_sampoo");
    _survey_treatment = survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_treatment");
  }

  let bom_code_array = bom_code.split(",");
  for (let code of bom_code_array) {
    if (_survey_sampoo.product_no == Number(product_no) && code.indexOf("CSP") > -1) {
      let variant_code = survey_variant_resource.shampoo.variant_code;
      await getCafe24Option(product_no, code, nick_name, variant_code, hash, quantity);
    }
...
async function surveyGetCafe24HairCareProducts() {
  ...
  let survey_sampoo_product_no = survey_sampoo.product_no;
  ...
  if (survey_discount_type && survey_discount_type == "employees") {
    survey_employees_sampoo = survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_sampoo");
    survey_employees_treatment = survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_treatment");
    survey_employees_set_product = survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_set_product");
    
    survey_sampoo_product_no = survey_employees_sampoo.product_no;
    survey_treatment_product_no = survey_employees_treatment.product_no;
    survey_set_product_product_no = survey_employees_set_product.product_no;
  }

  // 샴푸 상품 정보
  let shampoo = await surveyGetCafe24API(`/api/v2/products/${survey_sampoo_product_no}?embed=discountprice`);
  ...
  // 샴푸 품목 정보
  let shampoo_variant = await surveyGetCafe24API(`/api/v2/products/${survey_sampoo_product_no}/variants`);

...
function setSurveyScent(survey_target, params, befor_bom = null) {
  if (
    !params.osdl_result.point3.scent.name ||
    ...
    let set_html = `
      <div class="solution_cont">
        <div class="in">
          ...
          <img
            src="//ecimg.cafe24img.com/pg115b69008946048/cosmaxtest/web/upload/mynomy/kr/prd/ampoule01.png"
            alt="앰플이미지01"
          />
          <div class="explan">
...
window.addEventListener(
  ...
  async function (event) {
    setTimeout(async function () {
      ...
      // 이전문진 - 결과조회
      let survey_qna_result = await getSurveyDiagnosis(
        survey_hash,
      ...
      // 재문진 - 결과조회
      let survey_re_qna_result = await getSurveyDiagnosis(
        survey_re_qna_hash,
        survey_qna_at,
        dayjs().format("YYYY-MM-DD")
      );
      ...
      for (const product_id of survey_re_qna_result.product_id) {
        ...
        // 화면 노출
        setSurveyDiagnosis(survey_re_qna_result);
      }
      // 상품 가격 입력
      ...
      if (survey_product_no == survey_shampoo_no) {
        if (survey_product_resource.shampoo && survey_product_resource.shampoo.discountprice) {
      ...
      if (survey_product_no == survey_treatment_no) {
        if (survey_product_resource.shampoo && survey_product_resource.treatment.discountprice) {
          let price = Number(survey_product_resource.treatment.discountprice.pc_discount_price);
          let retail_price = Number(survey_product_resource.treatment.retail_price);
          survey_jQuery("#totalPrice .total strong em").text(surveyComma(price) + "원");
          survey_jQuery(".gdsBuyOpt .price strike").text(surveyComma(retail_price) + "원");
          survey_jQuery(".gdsBuyOpt .price em").text(surveyComma(price) + "원");
          if (retail_price <= price) {
      ...
      if (survey_product_no == survey_set_product_no) {
        if (
          survey_product_resource.set_product &&
          survey_product_resource.set_product.discountprice
        ) {
          let price = Number(survey_product_resource.set_product.discountprice.pc_discount_price);
          let retail_price = Number(survey_product_resource.set_product.retail_price);
          survey_jQuery("#totalPrice .total strong em").text(surveyComma(price) + "원");
          survey_jQuery(".gdsBuyOpt .price strike").text(surveyComma(retail_price) + "원");
          survey_jQuery(".gdsBuyOpt .price em").text(surveyComma(price) + "원");
          if (retail_price <= price) {
      ...
      // onchange="doSomething();" onkeypress="this.onchange();" onpaste="this.onchange();" oninput="this.onchange();"
    }, 100);
  },
  false
);

...
// 향 설명 리스트
let fragrance = [
  "초여름 청량하게 불어오는 바람에 실린<br>카멜리아 티향",
  "초여름 청량하게 불어오는 바람에 실린<br>카멜리아 티향",
  "초여름 청량하게 불어오는 바람에 실린<br>카멜리아 티향",
  "초여름 청량하게 불어오는 바람에 실린<br>카멜리아 티향",
  "초여름 청량하게 불어오는 바람에 실린<br>카멜리아 티향",
  "무향 설명입니다.<br>무향 설명입니다.",
];
...
let hair_loss_preset_question_subject_list = [
  "탈모 프리셋 - 남성, 합산점수 20 이상",
  "탈모 프리셋 - 여성, 합산점수 20 이상",
  "탈모 프리셋 - 남성, 합산점수 0 이상~20 미만",
  "탈모 프리셋 - 여성, 합산점수 0 이상~20 미만",
  "탈모 프리셋 - 남성, 합산점수 0 미만",
  "탈모 프리셋 - 여성, 합산점수 0 미만",
];

let hair_loss_base_array = [];
let hair_loss_base_question_subject_list = [
  { subject: "[미노출] 탈모 프리셋 - 남성, 합산점수 20 이상", key: "m_20_up" },
  { subject: "[미노출] 탈모 프리셋 - 여성, 합산점수 20 이상", key: "w_20_up" },
  { subject: "[미노출] 탈모 프리셋 - 남성, 합산점수 0 이상", key: "m_0_up" },
  { subject: "[미노출] 탈모 프리셋 - 여성, 합산점수 0 이상", key: "w_0_up" },
  { subject: "[미노출] 탈모 프리셋 - 남성, 합산점수 0 미만", key: "m_0_down" },
  { subject: "[미노출] 탈모 프리셋 - 여성, 합산점수 0 미만", key: "w_0_down" },
];

let treatment_answer_ampoule_info = {
  I: [
    "모발 윤기",
    "열손상 보호",
    "건성",
    "펌",
    "드라이기의 뜨거운 바람으로 머리를 말린다",
    "고데기나 드라이 스타일링을 주 3회 이상 한다",
  ],
  P: ["모발 부드러움", "정전기 방지"],
  H: ["모발 끊어짐 완화", "뿌리 볼륨", "지성", "탈색"],
  D: ["모발 보습"],
  M: ["염색모 케어", "염색"],
};

let survey_hair_descript = [
  { subject: "건강모", content: "손빗질시 엉킴이 전혀 없고<br>윤기있는 모발이며, 염색 이력 없음" },
  { subject: "손상모 1단계", content: "손빗질시 엉킴이 거의 없음" },
  { subject: "손상모 2단계", content: "손빗질시 엉킴이 있긴하지만<br>쉽게 풀어지는 편" },
  { subject: "손상모 3단계", content: "한 두번의 손빗질로는<br>쉽게 풀리지 않고 엉킴" },
  { subject: "극손상모", content: "엉킴이 심해 손빗질이 어렵고<br>모발 끝이 갈라진 모발이 많음" },
];
...
function setSurveyAnswer(
  ...
  if (answer_type == "text") {
    ...
    for (let i = 0; i < answer_list.length; i++) {
      if (answer_list[i].display_status == "enabled") {
        if (question_subject.indexOf("나의 모발 손상도") > -1) {
          let find_hair = survey_hair_descript.find((e) => e.subject == answer_list[i].subject);
          image_html += `
    ...
    if (is_hair) {
      ...
      // add_slide_class = "swiper-container-fade swiper-container-initialized swiper-container-vertical"; swiper-wrapper
    }
    ...
    answer_html += `</ul></div>`;
  } else if (answer_type == "width_slide_image") {
    fragrance_hierarchy_array = [];
    answer_html += `
    ...
    for (let i = 0; i < answer_list.length; i++) {
      if (answer_list[i].display_status == "enabled") {
        ...
        let aswer_subject = answer_list[i].subject.replace("- 강하게", "").trim();
        if (!fragrance_hierarchy_array[aswer_subject]) {
          fragrance_hierarchy_array[aswer_subject] = [];
        }
        fragrance_hierarchy_array[aswer_subject].push(answer_list[i].hierarchy_id);
        ...
        if (is_history_select) {
          line_checked = "line_checked";
          survey_question_answer_history_hierarchy_fragrance_name = answer_list[i].subject;
        }

        // 강하게는 미노출
        if (answer_list[i].subject.indexOf("- 강하게") > -1) {
          continue;
        }
        ...
        let scent_mid = "";
        let scent_bottom = "";
        let scent_find = survey_scent_list.find((e) => e.bulk_code == answer_list[i].bulk_code);
        ...
        answer_html += `
          <div class="swiper-slide ${line_checked}" answer_type="${answer_type}" min="${select_count.min}" max="${select_count.max}" hierarchy_id="${answer_list[i].hierarchy_id}" next_hierarchy_id="${next_hierarchy_id}" index="${i}" subject=${answer_list[i].subject}>
    ...
    let next_answer_html = "";
    let i = 0;
    for (let j = 0; j < survey.qna_hierarchy.length; j++) {
      if (survey.qna_hierarchy[j].children && survey.qna_hierarchy[j].children.length > 0) {
        for (const children of survey.qna_hierarchy[j].children) {
          if (Number(children.hierarchy_id) == Number(next_hierarchy_id)) {
            let count = 0;
            for (const answer of children.children) {
              let line_checked = "";
              checked = "";
              if (i == 0) {
                line_checked = "line_checked";
              ...
              if (answer.display_status == "enabled") {
                next_answer_html += `<input type="radio" name="strength" class="radiobox ${line_checked}" onClick="surveyRadioClick(this)" ${checked} hierarchy_id="${answer.hierarchy_id}" index="${i}"><label>${answer.subject}</label>`;
              }
              i++;
            }
          }
    ...
    let normal_radio_checked = "";
    let strong_radio_checked = "";
    let normal_radio_class = "";
    let strong_radio_class = "";
    if (survey_question_answer_history_hierarchy_fragrance_name.indexOf("강하게") > -1) {
      strong_radio_checked = "checked";
      strong_radio_class = "line_checked";
      survey_scent_type = "strong";
      // line_checked
    } else {
      normal_radio_checked = "checked";
      normal_radio_class = "line_checked";
    }

    let fix_asnwer_html = `
      <input type="radio" name="strength" class="radiobox ${normal_radio_class}" onClick="surveyRadioClick(this)" ${normal_radio_checked} value="normal"><label>보통</label>
      <input type="radio" name="strength" class="radiobox ${strong_radio_class}" onClick="surveyRadioClick(this)" ${strong_radio_checked} value="strong"><label>강하게</label>
    `;
    answer_html += `
            </div>
    ...
    for (let i = 0; i < answer_list.length; i++) {
      ...
      if (answer_list[i].display_status == "enabled") {
        ...
        if (is_history_select) {
          ...
          if (
            survey_question_answer_history_hierarchy_fragrance_name &&
            survey_question_answer_history_hierarchy_fragrance_name.indexOf("강하게") > -1
          ) {
            survey_question_answer_history_hierarchy_fragrance_name =
              answer_list[i].subject + " - 강하게";
          } else {
            survey_question_answer_history_hierarchy_fragrance_name = answer_list[i].subject;
          }
        }
    ...
    survey_scent_type = "";
    if (
      survey_worry_selected_list.indexOf("탈모 증상 완화") > -1 ||
      survey_worry_selected_list.indexOf("탈모증상완화") > -1
    ) {
      fix_asnwer_html = `
...
function showSurveyPreset(hierarchy_id, question) {
  ...
  for (let i = 0; i < question.presets.length; i++) {
    let hair_loss_preset_question_subject_index = hair_loss_preset_question_subject_list.findIndex(
      (e) => e == question.presets[i].subject
    );
    if (hair_loss_preset_question_subject_index > -1) {
    ...
    top_html += `
      <div class="swiper-slide" data-preset="${preset_text}">
  ...
  for (let i = 0; i < question.children.length; i++) {
    ...
    if (is_auto_select) {
      ...
      first_preset_text_answer.push(question.children[i].subject);
    }
    let is_question_history_select = surveyHistoryAutoSelectQuestion(
  ...
  for (let j = 0; j < survey.qna_hierarchy.length; j++) {
    if (survey.qna_hierarchy[j].children && survey.qna_hierarchy[j].children.length > 0) {
      ...
      for (let i = 0; i < children.length; i++) {
        if (Number(children[i].hierarchy_id) == Number(hierarchy_id)) {
          ...
          if (j == folder_sequence) {
            let subject_array = [];
            let bottom_subject = "";
            if (question_subject.indexOf("<br>") > -1) {
              subject_array = question_subject.split("<br>");
              if (subject_array.length > 0 && subject_array[0] == "나의 곱슬 정도는?") {
                question_subject = subject_array[0];
                subject_array.shift();
                bottom_subject = subject_array.join("");
              }
            }
      ...
      if (answer_type == "width_slide_image") {
        let index = null;
        let is_strong = false;
        if (survey_storage_history_hierarchy && survey_storage_history_hierarchy.fragrance_name) {
          let list = survey_jQuery(".fragranceSlide .swiper-wrapper .swiper-slide");
          for (let i = 0; i < list.length; i++) {
            let subject = survey_jQuery(list[i]).attr("subject");
            if (survey_storage_history_hierarchy.fragrance_name.indexOf("- 강하게") > -1) {
              is_strong = true;
              let fragrance_name = survey_storage_history_hierarchy.fragrance_name
                .replace("- 강하게", "")
                .trim();
              if (fragrance_name == subject) {
                index = i;
              }
            } else {
              if (survey_storage_history_hierarchy.fragrance_name == subject) {
                index = i;
              }
            }
          }
        ...
        if (
          survey_question_answer_history_hierarchy &&
          survey_question_answer_history_hierarchy_fragrance_name
        ) {
          let list = survey_jQuery(".fragranceSlide .swiper-wrapper .swiper-slide");
          for (let i = 0; i < list.length; i++) {
            ...
            if (survey_question_answer_history_hierarchy_fragrance_name.indexOf("- 강하게") > -1) {
              is_strong = true;
              let fragrance_name = survey_question_answer_history_hierarchy_fragrance_name
                .replace("- 강하게", "")
                .trim();
              if (fragrance_name == subject) {
                index = i;
              }
              survey_jQuery("input:radio[name='strength']:radio[value='strong']").click();
            } else {
              survey_jQuery("input:radio[name='strength']:radio[value='normal']").click();
              if (survey_question_answer_history_hierarchy_fragrance_name == subject) {
                index = i;
              }
            }
          }
        ...
        setTimeout(function () {
          settingWidthSlideImage(index);
          if (is_strong) {
            survey_jQuery("input[type=radio][value='strong']").prop("checked", true);
          }
        }, 50);
      }
      // 향 일때
...
function settingWidthSlideImage(index) {
  var fragranceSlide = new Swiper(".fragranceSlide", {
    ...
    on: {
      slideChangeTransitionStart: function () {
        let fragrance_text = survey_jQuery(".fragranceSlide .swiper-slide-active h4").text();
        survey_jQuery(".frag_gray").hide();

        if (fragrance_text == "무향") {
      ...
      slideChangeTransitionEnd: function () {
        ...
        let hierarchy_id = survey_jQuery(".swiper-slide-active").attr("hierarchy_id");
        let value = survey_jQuery("input[name='strength']:checked").val();
        if (fragrance_text == "무향") {
          value = "normal";
        }
        if (value == "strong") {
          for (const fragrance in fragrance_hierarchy_array) {
            if (fragrance_hierarchy_array[fragrance].indexOf(Number(hierarchy_id)) > -1) {
              setSurveyQuestionAnswerHistory(Number(fragrance_hierarchy_array[fragrance][1]), 1);
        ...
        if (
          ...
          if (data_preset) {
            ...
            for (let i = 0; i < preset.length; i++) {
              ...
              first_preset_text_answer.push($("#preset_" + preset[i]).text());
            ...
            // $(".select .view li").css("cursor", "default");
            $(".select .view li").css("cursor", "");
          }
        } else {
          $(".select .view li").css("cursor", "");
...
function surveyResult() {
  ...
  for (const hierarchy of survey.qna_hierarchy) {
    for (const hierarchy_children of hierarchy.children) {
      let base_index = hair_loss_base_question_subject_list.findIndex(
        (e) => e.subject == hierarchy_children.subject
      );
      if (base_index > -1) {
        if (!hair_loss_base_array[hair_loss_base_question_subject_list[base_index].key]) {
          hair_loss_base_array[hair_loss_base_question_subject_list[base_index].key] = [];
        }
        for (const answer of hierarchy_children.children) {
          hair_loss_base_array[hair_loss_base_question_subject_list[base_index].key].push(
            answer.hierarchy_id
          );
        }
      }
  ...
  if (is_set_product == true) {
    for (const hierarchy_id of survey_history_hierarchy) {
      ...
      if (formula) {
        ...
        if (
          formula.subject == "탈모증상완화" ||
          formula.subject == "탈모증상 완화" ||
          formula.subject == "탈모 증상 완화"
        ) {
          is_hair_loss = true;
        }
      }
    ...
    // 탈모 정보 추가
    let add_hair_loss_info_hierarchy_ids = addHairLossInfoHierarchyId(total_score);
    let delete_hierarchy_ids = [];
    if (is_hair_loss == true) {
      for (const hierarchy of survey.qna_hierarchy) {
        if (hierarchy.type == "folder") {
          for (const hierarchy_children of hierarchy.children) {
            if (
              hierarchy_children.subject == "해결하고 싶은 고민을\n선택해 주세요." ||
              hierarchy_children.subject == "해결하고 싶은 고민을 선택해 주세요." ||
              hierarchy_children.subject == "비듬 / 각질의 형태가 어떤가요?"
            ) {
              for (const hierarchy_deep_children of hierarchy_children.children) {
                delete_hierarchy_ids.push(hierarchy_deep_children.hierarchy_id);
              }
            }
          }
        }
      ...
      let add_survey_history_hierarchy = [];
      for (const history_hierarch_id of survey_history_hierarchy) {
        let delete_index = delete_hierarchy_ids.findIndex(
          (e) => Number(e) == Number(history_hierarch_id)
        );
        if (delete_index == -1) {
          add_survey_history_hierarchy.push(Number(history_hierarch_id));
        }
      }
        ...
        surveys: [
          {
            ...
            hierarchy_ids: survey_history_hierarchy,
            // bom_hierarchy_ids: [
            //   ...add_survey_history_hierarchy,
            //   ...add_hair_loss_info_hierarchy_ids,
            // ],
            bom_hierarchy_ids,
            // bom_hierarchy_ids: [...add_survey_history_hierarchy, ...base_hierarchy_ids],
          },
          {
            product_no: survey_treatment_no,
            hierarchy_ids: treatment_survey_history_hierarchy,
    ...
    // 탈모 정보 추가
    let add_hair_loss_info_hierarchy_ids = addHairLossInfoHierarchyId(total_score);
    let add_survey_hierarchy_ids = [];
    if (add_hair_loss_info_hierarchy_ids.length > 0) {
      for (const _hair_loss_info_hierarchy_id of add_hair_loss_info_hierarchy_ids) {
        let _parent_hierarchy_id = searchQuestionHierarchyId(_hair_loss_info_hierarchy_id);
        let childrens = searchQuestionChildren(_parent_hierarchy_id);
        for (const children of childrens) {
          let index = survey_hierarchy_ids.findIndex(
            (e) => Number(e) == Number(children.hierarchy_id)
          );
          if (index == -1) {
            add_survey_hierarchy_ids.push(children.hierarchy_id);
          }
        }
      }
    }
    ...
    if (is_hair_loss == true && survey_shampoo_no == survey_product_no) {
      ...
      set_param = {
        ...
        survey_hierarchy_ids,
        // bom_hierarchy_ids: [...add_survey_history_hierarchy, ...add_hair_loss_info_hierarchy_ids],
        bom_hierarchy_ids,
        // bom_hierarchy_ids: [...add_survey_history_hierarchy, ...base_hierarchy_ids],
        member: {
          product_name: product_name,
  ...
  if (survey_product_no == survey_treatment_no || survey_product_no == survey_set_product_no) {
    ...
    if (survey_product_no == survey_treatment_no) {
      ...
      for (const hierarchy_id of survey_history_hierarchy) {
        let formula = searchFormulaByHierarchyId(hierarchy_id);
        if (formula) {
          for (let answer_ampoule in treatment_answer_ampoule_info) {
            for (let answer of treatment_answer_ampoule_info[answer_ampoule]) {
              if (formula.subject == answer) {
      ...
      if (treatment_answer_ampoule_hierarchy_ids.length < 3) {
        ...
        if (index_type_P == -1) {
          ...
          if (index_p == -1) {
            ...
            if (bom_hierarchy_ids_index == -1) {
              set_param.surveys[1].bom_hierarchy_ids.push(Number(fix_answer_hierarchy_info.P));
            }
          }
        }
        ...
        if (index_type_D == -1) {
          ...
          if (index_d == -1) {
            ...
            if (bom_hierarchy_ids_index == -1) {
              set_param.surveys[1].bom_hierarchy_ids.push(Number(fix_answer_hierarchy_info.D));
            }
          }
        }
      } else if (treatment_answer_ampoule_hierarchy_ids.length == 3) {
  ...
  survey_jQuery.ajax({
    ...
    success: function (result) {
      ...
      // location.href = "/survey/result.html?product_no=" + survey_product_no;
      let gift_key = "";
      let product_type = "";
      if (survey_gift_key) {
...
function surveyAnswerPresetClick(element) {
  ...
  let first_preset_text_answer = [];
  let subject = survey_jQuery(".select_inner .txt h3").text().trim();
  if (subject.indexOf("해결하고 싶은 고민을") > -1) {
    if (select_answer_count > 0) {
      for (let element of survey_jQuery(".line_checked")) {
        first_preset_text_answer.push(survey_jQuery(element).text());
      }
    }
    localStorage.setItem("survey_worry_selected", JSON.stringify(first_preset_text_answer));
  }
...
function surveyRadioClick(element) {
  ...
  if (value == "strong") {
    if (
      ...
      survey_question_answer_history_hierarchy_fragrance_name =
        survey_question_answer_history_hierarchy_fragrance_name + " - 강하게";
    }
    survey_scent_type = "strong";
  } else {
    if (survey_question_answer_history_hierarchy_fragrance_name) {
      survey_question_answer_history_hierarchy_fragrance_name =
        survey_question_answer_history_hierarchy_fragrance_name.replace(" - 강하게", "").trim();
    }
    survey_scent_type = "";
  }
  setSurveyQuestionAnswerHistory(Number(hierarchy_id), 1);
...
// hierarchy_id로 formula_score 찾기
function addHairLossInfoHierarchyId(total_score) {
  if (is_hair_loss == true) {
    if (gender == "male") {
      if (total_score >= 20) {
        let hair_loss_preset_index = hair_loss_preset_array.findIndex(
          (e) => e.subject == "탈모 프리셋 - 남성, 합산점수 20 이상"
        );
        if (hair_loss_preset_index > -1) {
          return hair_loss_preset_array[hair_loss_preset_index].preset_answer_config;
        }
      } else if (total_score >= 0 && total_score < 20) {
        let hair_loss_preset_index = hair_loss_preset_array.findIndex(
          (e) => e.subject == "탈모 프리셋 - 남성, 합산점수 0 이상~20 미만"
        );
        if (hair_loss_preset_index > -1) {
          return hair_loss_preset_array[hair_loss_preset_index].preset_answer_config;
        }
        ...
        let hair_loss_preset_index = hair_loss_preset_array.findIndex(
          (e) => e.subject == "탈모 프리셋 - 남성, 합산점수 0 미만"
        );
        if (hair_loss_preset_index > -1) {
          return hair_loss_preset_array[hair_loss_preset_index].preset_answer_config;
        }
      ...
      if (total_score >= 20) {
        let hair_loss_preset_index = hair_loss_preset_array.findIndex(
          (e) => e.subject == "탈모 프리셋 - 여성, 합산점수 20 이상"
        );
        if (hair_loss_preset_index > -1) {
          return hair_loss_preset_array[hair_loss_preset_index].preset_answer_config;
        }
        ...
        let hair_loss_preset_index = hair_loss_preset_array.findIndex(
          (e) => e.subject == "탈모 프리셋 - 여성, 합산점수 0 이상~20 미만"
        );
        if (hair_loss_preset_index > -1) {
          return hair_loss_preset_array[hair_loss_preset_index].preset_answer_config;
        }
        ...
        let hair_loss_preset_index = hair_loss_preset_array.findIndex(
          (e) => e.subject == "탈모 프리셋 - 여성, 합산점수 0 미만"
        );
        if (hair_loss_preset_index > -1) {
          return hair_loss_preset_array[hair_loss_preset_index].preset_answer_config;
        }
...
// hierarchy_id로 formula_score 찾기
function addHairLossBaseHierarchyId(total_score) {
  if (is_hair_loss == true) {
    if (gender == "male") {
      if (total_score >= 20) {
        let hair_loss_base_index = hair_loss_base_array.findIndex(
          (e) => e.subject == "[미노출] 탈모 프리셋 - 남성, 합산점수 20 이상"
        );
        if (hair_loss_base_index > -1) {
          return hair_loss_base_array[hair_loss_base_index].preset_answer_config;
        }
      } else if (total_score >= 0 && total_score < 20) {
        let hair_loss_preset_index = hair_loss_preset_array.findIndex(
          (e) => e.subject == "탈모 프리셋 - 남성, 합산점수 0 이상~20 미만"
...
function setSurveyReQna() {
  ...
  for (let j = 0; j < survey.qna_hierarchy.length; j++) {
    if (
      survey.qna_hierarchy[j].type == "re_qna_folder" &&
      survey.qna_hierarchy[j].intro_status == "enabled"
    ) {
      let set_param = {
        sequence: survey.qna_hierarchy[j].sequence,
        html: survey.qna_hierarchy[j].intro_html,
      };
      survey_re_qna.push(set_param);
    }
  }
}
// 재문진 노출
function showSurveyReQna(sequence) {
  for (let j = 0; j < survey_re_qna.length; j++) {
    if (Number(survey_re_qna[j].sequence) == Number(sequence)) {
      survey_jQuery("#survey_area").empty();
      survey_jQuery("#survey_area").append(survey_re_qna[j].html);
      break;
    }
  }
}

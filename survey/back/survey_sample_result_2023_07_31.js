...
async function getSurveyConfig() {
  ...
  survey_sampoo = survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "sample_sampoo");
...
async function getSurveyAllDiagnoses() {
  ...
  let order_status_list = [
    "N00","N10","N22","N20","N21","N30","N40","N50"
  ];
  return new Promise(function (resolve, reject) {
    survey_jQuery.ajax({
        ...
        for (const diagnoses of result) {
          for (const diagnose of diagnoses) {
            if (diagnose.order_id) {
              let sample_order_list = await getSurveyDiagnosisOrder(diagnose.buy_at, diagnose.buy_at);
              for (const sample_order of sample_order_list) {
                if (sample_order.order_id == diagnose.order_id) {
                  let find_sample_order = sample_order.items.find((e) => Number(e.product_no) == Number(survey_product_no) && order_status_list.indexOf(e.order_status) > -1);
...
async function surveyCart(type) {
  ...
  for (const product of product_list) {
    ...
    if (product_type != "set_product") {
      // 샴푸, 트리트먼트
      surveySetHairCareCart(product_no, bom_code, nick_name, qna_type, hash, quantity, product_type);
    } else {
      // 세트 상품
      ...
      let shampoo_option_result = await getSurveyCafe24Option(survey_product_resource.shampoo.product_no);
      ...
      let treatment_option_result = await getSurveyCafe24Option(survey_product_resource.treatment.product_no);
...
async function surveyGetCafe24SampleProducts() {
  ...
  if (survey_is_sample_order) {
    ...
    let _survey_set_product = survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "set_product");
  ...
  // 샴푸 상품 정보
  let shampoo = await surveyGetCafe24API(`/api/v2/products/${_survey_shampoo_no}?embed=discountprice`);
  survey_product_resource.shampoo = shampoo.product;

  // 트리트먼트 상품 정보
  let treatment = await surveyGetCafe24API(`/api/v2/products/${_survey_treatment_no}?embed=discountprice`);
  survey_product_resource.treatment = treatment.product;

  // 세트 상품 정보
  let set_product = await surveyGetCafe24API(`/api/v2/products/${_survey_set_product_no}?embed=discountprice`);
  ...
  // 트리트먼트 품목 정보
  let treatment_variant = await surveyGetCafe24API(`/api/v2/products/${_survey_treatment_no}/variants`);
  survey_variant_resource.treatment = treatment_variant.variants[0];

  // 세트 품목 정보
  let set_product_variant = await surveyGetCafe24API(`/api/v2/products/${_survey_set_product_no}/variants`);
  ...
  async function (event) {
    setTimeout(async function () {
      ...
      setTimeout(async function () {
        ...
        let pc_discount_price = Number(survey_product_resource.set_product.discountprice.pc_discount_price);

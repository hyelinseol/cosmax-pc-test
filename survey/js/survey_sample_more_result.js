...
function surveyInputBomCode(result, type = null) {
  ...
  // let survey_product = survey_product_list[CAFE24API.MALL_ID].find((e) => Number(e.product_no) == Number(result.product_no));
  ...
  console.log("survey_product_resource.treatment.product_no", survey_product_resource.treatment.product_no);
...
function surveyQuantity(element, type) {
  ...
  survey_jQuery(".survey_popup_price .total em").text(surveyComma(total_price) + "원");
...
function surveySetQuantityClick(type) {
  if (survey_product_no == survey_set_product_no) {
    ...
    setTimeout(function () {
      ...
      let price_text = survey_jQuery(".subscribeKindWrap .subscribeBox .gdsBuyOpt .price em").text();
      const regex = /[^0-9]/g;
      const price = price_text.replace(regex, "");

      // survey_jQuery(".sample_price").text(surveyComma(price * count) + "원");

      survey_jQuery(".survey_quantity").text(count);
    }, 200);
  }
  setTimeout(function () {
    // survey_jQuery(".sample_price").text(surveyComma(Number(survey_product_resource.shampoo.discountprice.mobile_discount_price)) + "원");
  }, 400);
}
...
function surveyBuyBtn() {
  ...
  survey_jQuery(".bottom_sample_price").addClass("displaynone");
  survey_jQuery(".sample_price").show();

  survey_jQuery(".survey_popup_price").css("display", "flex");
  survey_jQuery(".survey_bottom_price").hide();
  survey_jQuery(".disPrice").hide();
}

function surveyCloseBtn() {
  survey_jQuery(".sample_price").hide();
  survey_jQuery(".bottom_sample_price").removeClass("displaynone");
...
function surveyProductSelect(type) {
  ...
  let html = `
  <div class="subscribeBox" product_no="${product_no}" bom_code="${bom_code}" nick_name="${nick_name}" qna_type="${qna_type}" hash="${hash}" payment_buy_type="${survey_buy_type_value}" regular_cycle="${survey_regular_cycle}" count="1" price="${pc_discount_price}">
...
window.addEventListener(
  ...
  async function (event) {
    setTimeout(async function () {
      ...
      setTimeout(async function () {
        let price = Number(survey_product_resource.shampoo.price);
        let pc_discount_price = Number(survey_product_resource.shampoo.discountprice.pc_discount_price);
        let discount = Math.floor(((price - pc_discount_price) / price) * 100);
        survey_jQuery(".disPrice").text(discount + "%");
        if (discount <= 0) {
          survey_jQuery(".disPrice").addClass("displaynone");
        }
        survey_jQuery(".sample_price_strike").text(surveyComma(price));
        survey_jQuery(".sample_price").text(surveyComma(pc_discount_price) + "원");
        survey_jQuery(".bottom_sample_price").text(surveyComma(pc_discount_price) + "원");
        if (pc_discount_price <= price) {
          survey_jQuery(".sample_price_strike").hide();
        } else {
          survey_jQuery(".subscribeBox .gdsBuyOpt .price strike").text(surveyComma(price) + "원");
        }
        survey_jQuery(".subscribeBox .gdsBuyOpt .price em").text(surveyComma(pc_discount_price) + "원");

        survey_jQuery(".total-price-wrap").removeClass("displaynone");
        survey_jQuery(".survey_bottom_price").removeClass("displaynone");
      }, 1000);

...
async function getSurveyConfig() {
  ...
  survey_shampoo = surveyCommon.survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "shampoo");
  survey_treatment = surveyCommon.survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "treatment");
  survey_set_product = surveyCommon.survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "set_product");

  survey_shampoo_no = survey_shampoo.no;
  survey_treatment_no = survey_treatment.no;
  survey_set_product_no = survey_set_product.no;

  console.log("survey_set_product_no", survey_set_product_no);
  // 세트 상품일 때
  ...
  if (!product_no || !bom_code || !nick_name || !qna_type || !hash) {
    ...
    if (add_result_diagnoses) {
      ...
      if (survey_discount_type && survey_discount_type.indexOf("employees") > -1) {
        ...
        if (survey_employees_shampoo && type == "shampoo") {
          product_no = survey_employees_shampoo.no;
        }
        if (survey_employees_treatment && type == "treatment") {
          product_no = survey_employees_treatment.no;
        }
        if (survey_employees_set_product && type == "set_product") {
...
async function surveyCart(type) {
  ...
  if (survey_discount_type && survey_discount_type.indexOf("employees") > -1) {
    let employees_set_product = surveyCommon.survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_set_product" && e.detail_type == survey_discount_type);
    if (employees_set_product) {
      _survey_set_product_no = employees_set_product.no;
    }
    let employees_shampoo_product = surveyCommon.survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_shampoo" && e.detail_type == survey_discount_type);
    if (employees_shampoo_product) {
      _survey_shampoo_product_no = employees_shampoo_product.no;
    }
    let employees_treatment_product = surveyCommon.survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_treatment" && e.detail_type == survey_discount_type);
  ...
  for (const product of product_list) {
    ...
    if (product_no != _survey_set_product_no) {
      // 샴푸, 트리트먼트
      await surveySetHairCareCart(product_no, bom_code, nick_name, qna_type, hash, quantity);
    } else {
      // 세트 상품
...
async function surveySetHairCareCart(
  ...
  let _survey_shampoo = surveyCommon.survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "shampoo");
  let _survey_treatment = surveyCommon.survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "treatment");

  if (survey_discount_type && survey_discount_type.indexOf("employees") > -1) {
    _survey_shampoo = surveyCommon.survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_shampoo" && e.detail_type == survey_discount_type);
    _survey_treatment = surveyCommon.survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_treatment" && e.detail_type == survey_discount_type);
  }
...
async function surveyGetCafe24HairCareProducts() {
  ...
  let survey_shampoo_product_no = survey_shampoo.no;
  let survey_treatment_product_no = survey_treatment.no;
  let survey_set_product_product_no = survey_set_product.no;

  if (survey_discount_type && survey_discount_type.indexOf("employees") > -1) {
    survey_employees_shampoo = surveyCommon.survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_shampoo" && e.detail_type == survey_discount_type);
    survey_employees_treatment = surveyCommon.survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_treatment" && e.detail_type == survey_discount_type);
    survey_employees_set_product = surveyCommon.survey_product_list[CAFE24API.MALL_ID].find((e) => e.product_type == "employees_set_product" && e.detail_type == survey_discount_type);
  ...
  // 샴푸 상품 정보
  let shampoo = await surveyGetCafe24API(`/api/v2/products/${survey_shampoo_product_no}?embed=discountprice`);
  survey_product_resource.shampoo = shampoo.product;


  // 트리트먼트 상품 정보
  let treatment = await surveyGetCafe24API(`/api/v2/products/${survey_treatment_product_no}?embed=discountprice`);
  survey_product_resource.treatment = treatment.product;

  // 세트 상품 정보
  let set_product = await surveyGetCafe24API(`/api/v2/products/${survey_set_product_product_no}?embed=discountprice`);
  survey_product_resource.set_product = set_product.product;


  // 샴푸 품목 정보
  let shampoo_variant = await surveyGetCafe24API(`/api/v2/products/${survey_shampoo_product_no}/variants`);
  survey_variant_resource.shampoo = shampoo_variant.variants[0];

  // 트리트먼트 품목 정보
  let treatment_variant = await surveyGetCafe24API(`/api/v2/products/${survey_treatment_product_no}/variants`);
  survey_variant_resource.treatment = treatment_variant.variants[0];

  // 세트 품목 정보
  let set_product_variant = await surveyGetCafe24API(`/api/v2/products/${survey_set_product_product_no}/variants`);
  ...
  async function (event) {
    ...
    setTimeout(async function () {
      ...
      if (hash) {
        ...
        if (!localStorage.getItem("survey_worry_shampoo_selected") || !localStorage.getItem("survey_worry_treatment_selected")) {
          await surveyHeirCareSleep(500);
        }
      ...
      setTimeout(async function () {
        ...
        if (!survey_product_resource?.shampoo?.product_name || survey_product_resource?.treatment?.product_name || survey_product_resource?.set_product?.product_name) {
          await surveyGetCafe24HairCareProducts();
          await surveyHeirCareSleep(500);
        }
        if (survey_discount_type && survey_discount_type.indexOf("employees") > -1) {
        ...
        */
        if (survey_discount_type && survey_discount_type.indexOf("employees") > -1) {
          ...
          if (buy_type) {
            ...
            const price = surveyComma(Number(survey_product_resource[buy_type].discountprice.pc_discount_price)) + "원";

...
let hair_loss_preset_question_hierarchy_id_list = {
  cosmaxtest: [6335, 6341, 6347, 6353, 6359, 6365],
  threewaau: [4448, 4454, 4460, 4466, 4472, 4478],
};
...
let hair_loss_base_question_hierarchy_id_list = {
  cosmaxtest: [
    { key: "m_20_up", hierarchy_id: 6335 },
    { key: "w_20_up", hierarchy_id: 6341 },
    { key: "m_0_up", hierarchy_id: 6347 },
    { key: "w_0_up", hierarchy_id: 6353 },
    { key: "m_0_down", hierarchy_id: 6359 },
    { key: "w_0_down", hierarchy_id: 6365 },
  ],
  threewaau: [
    { key: "m_20_up", hierarchy_id: 4448 },
    { key: "w_20_up", hierarchy_id: 4454 },
    { key: "m_0_up", hierarchy_id: 4460 },
    { key: "w_0_up", hierarchy_id: 4466 },
    { key: "m_0_down", hierarchy_id: 4472 },
    { key: "w_0_down", hierarchy_id: 4478 },
  ],
};

let treatment_answer_ampoule_type_info = {
  cosmaxtest: {
    I: [6511, 6517, 6523, 6576, 6591, 6592],
    P: [6512, 6518],
    H: [6513, 6515, 6521, 6578],
    D: [6514],
    M: [6516, 6577],
  },
  threewaau: {
    I: [4638, 4644, 4650, 4703, 4718, 4719],
    P: [4639, 4645],
    H: [4640, 4642, 4648, 4705],
    D: [4641],
    M: [4643, 4704],
  },
};
  ...
  cosmaxtest: [
    {
      hierarchy_id: 6295,
      subject: "건강모",
      content: "손빗질시 엉킴이 전혀 없고<br>윤기있는 모발이며, 염색 이력 없음",
    },
    { hierarchy_id: 6296, subject: "손상모 1단계", content: "손빗질시 엉킴이 거의 없음" },
    {
      hierarchy_id: 6297,
      subject: "손상모 2단계",
      content: "손빗질시 엉킴이 있긴하지만<br>쉽게 풀어지는 편",
    },
    {
      hierarchy_id: 6298,
      subject: "손상모 3단계",
      content: "한 두번의 손빗질로는<br>쉽게 풀리지 않고 엉킴",
    },
    {
      hierarchy_id: 6299,
      subject: "극손상모",
      content: "엉킴이 심해 손빗질이 어렵고<br>모발 끝이 갈라진 모발이 많음",
    },
  ],
  threewaau: [
    {
    ...
    { hierarchy_id: 4409, subject: "손상모 1단계", content: "손빗질시 엉킴이 거의 없음" },
    {
      hierarchy_id: 4410,
      subject: "손상모 2단계",
      content: "손빗질시 엉킴이 있긴하지만<br>쉽게 풀어지는 편",
...
let no_show_hierarchy = [
  6335, 6341, 6347, 6321, 6353, 6359, 6365, 6609, 4448, 4454, 4460, 4434, 4466, 4472, 4478, 4736,
];
...
async function getSurveyConfig() {
  ...
  survey_shampoo = survey_product_list[CAFE24API.MALL_ID].find(
    (e) => e.product_type == "sample_shampoo"
  );
...
function setSurveyAnswer(
  ...
  if (answer_type == "text") {
    ...
    for (let i = 0; i < answer_list.length; i++) {
      if (answer_list[i].display_status == "enabled") {
        if (survey_my_hair_damage[survey_mall_id].indexOf(Number(question.hierarchy_id)) > -1) {
          let find_hair = survey_hair_descript[survey_mall_id].find(
            (e) => Number(e.hierarchy_id) == Number(answer_list[i].hierarchy_id)
          );
          image_html += `
    ...
    let normal_class = "";
    let strong_class = "";
    if (survey_scent_type == "strong" || is_strong == true) {
    ...
    let strength_class = "";
    survey_scent_type = "";
    if (survey_worry_selected_list.indexOf(survey_hair_loss_relax[survey_mall_id]) > -1) {
      fix_asnwer_html = `
...
function showSurveyPreset(hierarchy_id, question) {
  ...
  for (let i = 0; i < question.presets.length; i++) {
    let hair_loss_preset_question_hierarchy_id_index = hair_loss_preset_question_hierarchy_id_list[
      survey_mall_id
    ].findIndex((e) => e == Number(question.presets[i].hierarchy_id));
...
function surveyResult() {
  ...
  for (const hierarchy of survey.qna_hierarchy) {
    for (const hierarchy_children of hierarchy.children) {
      ...
      if (base_index > -1) {
        if (
          !hair_loss_base_array[
            hair_loss_base_question_hierarchy_id_list[survey_mall_id][base_index].key
          ]
        ) {
          hair_loss_base_array[
            hair_loss_base_question_hierarchy_id_list[survey_mall_id][base_index].key
  ...
  if (is_set_product == true) {
    for (const hierarchy_id of survey_history_hierarchy) {
      ...
      if (formula) {
        ...
        if (Number(survey_hair_loss_relax[survey_mall_id]) == Number(formula.hierarchy_id)) {
          is_hair_loss = true;
        }
      }
    }
  ...
  survey_jQuery.ajax({
    ...
    success: function (result) {
      ...
      location.href =
        "/survey/sample_result.html?product_no=" +
        survey_product_no +
        "&hash=" +
        result[0].hash +
        "&qna_at=" +
...
function surveyAnswerPresetClick(element) {
  ...
  let first_preset_text_answer = [];
  let question_hierarchy_id = Number(
    survey_jQuery(".survey_worry_question.swiper-slide-active").attr("hierarchy_id")
  );
...
// 문진 리스트 조회
async function getSurveyResultByMemberId() {
  let from = dayjs("2023-02-01").format("YYYY-MM-DD");
  let to = dayjs().format("YYYY-MM-DD");
  return new Promise(function (resolve, reject) {
    survey_mall_id = CAFE24API.MALL_ID;
    survey_shop_no = CAFE24API.SHOP_NO;
    survey_jQuery.ajax({
      url: `${survey_domain}/app/${survey_app_name}/mall/${survey_mall_id}/api/survey/shops/${survey_shop_no}/front/diagnoses?member_id=${survey_member_id}&from=${from}&to=${to}`,
      type: "get",
      data: {},
      dataType: "json",
      success: function (result) {
...
function surveySampleOrderCheck(order_list) {
  ...
  for (const order of order_list) {
    if (find_product) {
      if ((Number(order[0]?.product_no) == Number(find_product.product_no) || Number(order[1]?.product_no) == Number(find_product.product_no)) && (order[0]?.order_id || order[1]?.order_id)) {
        return true;
      }
    }
  }
  ...
  async function (event) {
    (async function (CAFE24API) {
      setTimeout(async function () {
        ...
        const survey_result_list = await getSurveyResultByMemberId();
        let is_order = surveySampleOrderCheck(survey_result_list);
        if (is_order == true) {
          alert("헤어케어 체험 키트는 1회 구매만 가능합니다.");
          history.back();
          return;
        }

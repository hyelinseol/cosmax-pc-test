...
for (const tr_basket_product of tr_basket_product_list) {
  ...
  if (find_product && find_product.product_type.indexOf("sample") > -1) {
    // 수량 입력 방지
    survey_jQuery(tr_basket_product).children(".ec-base-qty").children("input").attr("readonly", true);
    survey_jQuery(tr_basket_product).children(".ec-base-qty").children(".up").attr("onclick", "surveyPreventDefault(event);");
    survey_jQuery(tr_basket_product).children(".ec-base-qty").children(".up").attr("href", "javascript:void(0)");
    survey_jQuery(tr_basket_product).children(".ec-base-qty").children(".down").attr("onclick", "surveyPreventDefault(event);");
    survey_jQuery(tr_basket_product).children(".ec-base-qty").children(".down").attr("href", "javascript:void(0)");
  ...
  async function (event) {
    ...
    for (let i = 0; i < product_area_list.length; i++) {
      ...
      if (href) {
        ...
        survey_jQuery(product_area_list[i]).find(".odrPrdSet .prdGdsInfo .ec-product-name").attr("href", href);
      }
    }
